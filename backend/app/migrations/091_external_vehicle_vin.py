...
Numbered 091 on fork main (088 already creates external_vehicles; 085–090 are
Tier/family migrations). Idempotent: skips when the column already exists
(fresh installs that got vin from an updated 088 CREATE).
"""
...
def upgrade(engine=None):
    ...
    with engine.begin() as conn:
        ...
        if "external_vehicles" not in inspector.get_table_names():
            print("  → external_vehicles missing; skip (run 088 first)")
            return
